    ...
    public bool Verify(ReadOnlySpan<byte> data, ReadOnlySpan<byte> signature)
    {
        switch (_type)
        {
            case COSE.KeyType.EC2:
                // ***
                // apparentely ECDsa.Create below is not supported in WASM (at least not on Mac) so let's disable it temporarily,
                // which is not that terrible because we don't authenticate over the network 
