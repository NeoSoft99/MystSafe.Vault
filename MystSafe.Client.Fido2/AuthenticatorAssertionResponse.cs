...
public sealed class AuthenticatorAssertionResponse : AuthenticatorResponse
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        // *** debug platform exception thrown by "var rpid = Raw.Extensions?.AppID ?? false ? options.Extensions?.AppID : options.RpId;" line

        string rpid;
        try
        {
            var rawExtensions = Raw.Extensions;
            var rawAppID = rawExtensions?.AppID;
            var optionsExtensions = options.Extensions;
            var optionsAppID = optionsExtensions?.AppID;
            rpid = (rawAppID ?? false) ? optionsAppID : options.RpId;
            //throw new Exception("Debug exception: rpid_temp = " + rpid_temp ?? "empty");
        }
        catch (Exception e)
        {
            throw new Exception("Debug exception: " + e.Message);
        }
        // ***
        ...
        // *** platform exception fix
        //if (Raw.Extensions?.DevicePubKey is not null)
        if (Raw.Extensions is not null && Raw.Extensions?.DevicePubKey is not null)
        // ***
        {
